...
export const POST: APIRoute = async (context) => {
  ...
  const publishResult = await prepared.supabase.rpc("publish_offer_change_checked", {
    ...
    p_expected_pending_change_id: prepared.request.expectedPendingChangeId,
    p_supersession_confirmed: prepared.request.supersessionConfirmed,
  });
  if (publishResult.error) {
    const conflicts = new Set([
      ...
      "Pending proposal changed; reload before publishing",
    ]);
    ...
    if (publishResult.error.message === "Confirm replacement of the pending proposal")
      return json(400, { error: "Confirm that this change will replace the pending proposal." });
